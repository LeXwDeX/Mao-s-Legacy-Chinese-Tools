...
只复制下表列出的 **23 个文件** 到游戏目录。不要复制输出目录中未列出的实验文件（例如 `sharedassets2.assets`），它们不属于发布补丁，可能导致菜单异常或无法进入新游戏。
...
3. 是否修改过游戏其他文件
4. 是否复制了部署清单以外的文件；如复制过，先通过 Steam 验证游戏文件完整性，再只按上表部署
...
- **不要把实验性资源放进 `1.8.5_output/`**；发布物必须与玩家部署表逐项一致
...
```
.
├── 1.8.5_output/              # ← 汉化成品 (23 个文件，可直接部署)
