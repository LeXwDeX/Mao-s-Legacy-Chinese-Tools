...
public class CitizenManager : MonoBehaviour
{
	...
	public string GetTraitTranslation(PrimaryTrait trait)
	{
		return trait switch
		{
			PrimaryTrait.LeftRadical => translateScript.GetTranslation("左派激进分子", "Левый радикал"), 
	...
	public string GetTraitTranslation(TertiaryTrait trait)
	{
		return trait switch
		{
			TertiaryTrait.Pettytyrant => translateScript.GetTranslation("小暴君", "Самодур"), 
	...
	public void PromoteToPolitic(int slot)
	{
		...
		persona.changeLog.Add(FormatLog(persona, "был возвышен до политика", "被提拔为政治人物", date));
	}
	...
	public void CheckCareerDegradation(Persona citizen, int slot)
	{
		...
		if ((num4 || flag) && num3 > 80f)
		{
			...
			if (num6 && (citizen.status == Job.LocalPartyBranchChief || citizen.status == Job.RegionalPartyBranchChief || citizen.status == Job.SignificantPartyMember || citizen.status == Job.CorporationOwner))
			{
				...
				citizen.changeLog.Add(FormatLog(citizen, "был посажен в тюрьму из-за критической деградации!", "因严重堕落而被监禁！", date));
			}
			else if (citizen.status == Job.SmallBusinessOwner || citizen.status == Job.Farmer || citizen.status == Job.Worker)
			{
				...
				citizen.changeLog.Add(FormatLog(citizen, "стал безработным из-за критической деградации!", "因严重堕落而失业！", date));
			}
			else if (flag2 && citizen.Intrigue >= 7)
			{
				...
				citizen.changeLog.Add(FormatLog(citizen, "ушёл в подполье из-за критической деградации!", "因严重堕落而隐蔽起来！", date));
			}
		}
	}
	...
	public void CheckSpecialStatuses(Persona citizen, int slot)
	{
		if (citizen != null && !citizen.isPolitic)
		{
			...
			if (flag3 && UnityEngine.Random.value < 0.2f && !flag2)
			{
				...
				citizen.changeLog.Add(FormatLog(citizen, "попал в тюрьму из-за политической обстановки!", "因政治形势而被监禁！", date));
			}
			else if (((citizen.Intrigue >= 7 && (num == 1 || num == 2 || num == 3 || num == 0)) || num4 > 80f) && UnityEngine.Random.value < 0.1f && citizen.status != Job.Underground)
			{
				...
				citizen.changeLog.Add(FormatLog(citizen, "ушёл в подполье из-за политической обстановки!", "因政治形势而隐蔽起来！", date));
			}
			else if (flag2 && (!flag || num4 < 40f) && UnityEngine.Random.value < 0.2f)
			{
				...
				citizen.changeLog.Add(FormatLog(citizen, "был амнистирован!", "获得特赦！", date));
			}
		}
	}
		...
		if (citizen.Children >= 4 && citizen.Children <= 5)
		{
			if (citizen.Wealth <= 4)
			{
				if (UnityEngine.Random.value < 0.25f)
				{
					...
					citizen.changeLog.Add(FormatLog(citizen, "потерял работу из-за бедности и большого количества детей!", "因贫困与子女众多而丢了工作！", date));
					return;
				}
				num6 -= 10f;
			}
	...
	public void CheckUnemployedTransitions(Persona citizen, int slot)
	{
		...
		if (citizen.status == Job.Amnestied)
		{
			...
			{
				citizen.status = Job.Unemployed;
				citizen.changeLog.Add(FormatLog(citizen, "стал безработным после амнистии", "特赦后沦为失业者", date));
				Debug.Log(citizen.name + " " + citizen.surname + "从“特赦”转为“失业”");
			}
		}
		else
		{
			...
			if (!(UnityEngine.Random.value > 0.25f))
			{
				...
				if (list.Count > 0)
				{
					...
					citizen.changeLog.Add(FormatLog(citizen, "вышел из безработицы и теперь " + translateScript.GetJobTranslation(citizen.status), "离开失业，成为 " + translateScript.GetJobTranslation(citizen.status), date));
				}
			}
		}
	}
	...
	public void CheckCareerPathTransitions(Persona citizen, int slot)
	{
		...
		case Job.SmallBusinessOwner:
			if (citizen.Charisma >= 7 && citizen.Intrigue >= 6 && citizen.Wealth >= 6 && (num == 1 || num == 2 || num == 3))
			{
				...
				citizen.changeLog.Add(FormatLog(citizen, "перешёл из бизнеса и теперь " + translateScript.GetJobTranslation(citizen.status), "从经商转为 " + translateScript.GetJobTranslation(citizen.status), date));
			}
			else if (flag && citizen.Intrigue < 4 && citizen.Charisma < 4 && num5 > 70f)
			{
				...
				citizen.changeLog.Add(FormatLog(citizen, "был арестован как бизнесмен в авторитарном режиме", "在专制体制下，因商人身份被捕", date));
			}
			break;
		case Job.LocalPartyBranchChief:
			if ((num == 5 || num == 4) && citizen.Charisma >= 6 && citizen.Wealth >= 7)
			{
				...
				citizen.changeLog.Add(FormatLog(citizen, "перешёл из партии и теперь " + translateScript.GetJobTranslation(citizen.status), "从党内转为 " + translateScript.GetJobTranslation(citizen.status), date));
			}
			break;
		case Job.CorporationOwner:
			if ((num == 0 || num == 1 || num == 3) && citizen.Wealth >= 9 && citizen.Charisma >= 6)
			{
				...
				citizen.changeLog.Add(FormatLog(citizen, "перешёл от корпорации к " + translateScript.GetJobTranslation(citizen.status), "从公司转为 " + translateScript.GetJobTranslation(citizen.status), date));
			}
			break;
		case Job.RegionalPartyBranchChief:
			if (num == 5 && num2 > 80 && citizen.Wealth >= 8 && citizen.age < 45)
			{
				...
				citizen.changeLog.Add(FormatLog(citizen, "перешёл из регионального партийного начальника к " + translateScript.GetJobTranslation(citizen.status), "从地方党政负责人调任为 " + translateScript.GetJobTranslation(citizen.status), date));
			}
			break;
		case Job.Underground:
			if (citizen.Intrigue >= 8 && (num == 4 || num == 5 || num5 < 50f))
			{
				...
				citizen.changeLog.Add(FormatLog(citizen, "был реабилитирован и стал " + translateScript.GetJobTranslation(citizen.status), "获得平反并成为 " + translateScript.GetJobTranslation(citizen.status), date));
			}
			break;
		case Job.Worker:
		case Job.Farmer:
			if (num2 > 60 && citizen.Wealth >= 6 && citizen.age < 50 && !flag)
			{
				...
				citizen.changeLog.Add(FormatLog(citizen, "перешёл в " + translateScript.GetJobTranslation(citizen.status), "转为 " + translateScript.GetJobTranslation(citizen.status), date));
			}
			break;
		case Job.Peasant:
			if ((num == 1 || num == 3 || num == 2) && citizen.Wealth >= 4 && citizen.age > 30 && citizen.Children >= 2)
			{
				...
				citizen.changeLog.Add(FormatLog(citizen, "пошёл в " + translateScript.GetJobTranslation(citizen.status), "去了" + translateScript.GetJobTranslation(citizen.status), date));
			}
			break;
		case Job.SignificantPartyMember:
			if (num5 > 80f && citizen.Intrigue >= 8 && citizen.Charisma < 5 && flag)
			{
				...
				citizen.changeLog.Add(FormatLog(citizen, "ушёл в подполье из-за репрессий", "因镇压而隐蔽起来", date));
			}
			break;
		case Job.Prisoned:
		case Job.Amnestied:
	...
	private void TryRandomForeignContact(Persona citizen, int slot)
	{
		if (globalScript.gameState.data[14] == 5 && citizen.Charisma > 6 && citizen.Intrigue < 6 && !(UnityEngine.Random.value > 0.03f))
		{
			...
			int[] date = CurrentDate();
			citizen.changeLog.Add(FormatLog(citizen, "познакомился с иностранцем, узнал новое", "结识外国人，获得新见解", date));
			if (!citizen.tertiaryTraits.Contains(TertiaryTrait.Westophilic) && UnityEngine.Random.value < 0.05f)
			{
				citizen.tertiaryTraits.Add(TertiaryTrait.Westophilic);
				citizen.changeLog.Add(FormatLog(citizen, "проникся западными ценностями", "对西方价值观产生同情", date));
			}
		}
	}
	...
	{
		citizen.changeLog.Add(FormatLog(citizen, "стал объектом слежки из-за подозрительно высоких доходов", "因收入异常丰厚而遭到监视", CurrentDate()));
	...
	private void EndSurveillanceEvent(Persona citizen, int slot)
	{
		...
		if (wealth < num)
		{
			citizen.changeLog.Add(FormatLog(citizen, "слежка прекращена, так как его доходы снизились", "随着收入下降，监视结束", CurrentDate()));
		}
		else if (wealth > num)
		{
			if (UnityEngine.Random.value < 0.5f)
			{
				if (citizen.isPolitic || citizen.status == Job.SignificantPartyMember || citizen.status == Job.RegionalPartyBranchChief || citizen.status == Job.LocalPartyBranchChief)
				{
					...
					citizen.changeLog.Add(FormatLog(citizen, "стал фигурантом коррупционного скандала, но смог выкрутиться без серьезных последствий", "卷入贪腐丑闻，但得以脱身而未造成严重后果", CurrentDate()));
				}
				else
				{
					citizen.Wealth = Mathf.Max(citizen.Wealth - 3, 0);
					citizen.status = Job.Prisoned;
					citizen.changeLog.Add(FormatLog(citizen, "был арестован по подозрению в коррупции и потерял часть состояния", "因贪腐指控被捕，并损失部分财产", CurrentDate()));
				}
			}
			else
			{
				citizen.changeLog.Add(FormatLog(citizen, "слежка завершена без последствий", "监视无果而终止", CurrentDate()));
			}
		}
	...
	private void PartyPurgeEvent(Persona citizen, int slot)
	{
		...
		if (value < num6)
		{
			...
			citizen.changeLog.Add(FormatLog(citizen, "поддержал партийную чистку, укрепив своё положение в партии.", "支持党内清洗，巩固自身地位。", date));
		...
		else if (value < num6 + num7 && !citizen.isPolitic)
		{
			...
			citizen.changeLog.Add(FormatLog(citizen, "обвинён в саботаже во время чистки и арестован.", "在清洗期间被指控破坏活动并被捕。", date));
			Debug.Log(citizen.name + " " + citizen.surname + " провалил чистку и был репрессирован.");
		}
		else
		{
			citizen.changeLog.Add(FormatLog(citizen, "уклонился от участия в чистке, сохранив нейтралитет.", "避开参与清洗，保持中立。", date));
	...
	private void EconomicReformEvent(Persona citizen, int slot)
	{
		...
		if (num4 && flag && flag2 && flag3 && UnityEngine.Random.value < 0.15f)
		{
			...
			if (value < 0.6f)
			{
				citizen.status = Job.SmallBusinessOwner;
				citizen.Wealth += 2;
				citizen.changeLog.Add(FormatLog(citizen, "воспользовался реформами и открыл малый бизнес.", "借改革之机开起小生意。", date));
			}
			else if (value < 0.9f)
			{
				citizen.Wealth = Mathf.Max(0, citizen.Wealth - 1);
				citizen.changeLog.Add(FormatLog(citizen, "попытался открыть бизнес, но потерпел неудачу.", "试图创业，但失败了。", date));
			}
			else if (num == 0 || num == 1)
			{
				citizen.status = Job.Prisoned;
				citizen.changeLog.Add(FormatLog(citizen, "обвинён в капиталистическом уклоне и арестован.", "被指控走资本主义道路倾向并被监禁。", date));
			}
			else
			{
				citizen.Wealth = Mathf.Max(0, citizen.Wealth - 1);
	...
	private void RuralCrisisEvent(Persona citizen, int slot)
	{
		...
		if (value < 0.5f)
		{
			...
			{
				citizen.status = Job.Unemployed;
				citizen.changeLog.Add(FormatLog(citizen, "стал безработным из-за разорения", "因破产而失业", date));
			}
		}
		else if (value < 0.8f)
		{
			...
			{
				citizen.Wealth = Mathf.Min(citizen.Wealth + 1, 15);
				citizen.changeLog.Add(FormatLog(citizen, "получил помощь от государства в условиях кризиса из-за неурожая", "因歉收危机，得到国家救助", date));
			}
		}
		else
		{
	...
	private void RumorOfConspiracyEvent(Persona citizen, int slot)
	{
		...
		{
			int[] date = CurrentDate();
			citizen.changeLog.Add(FormatLog(citizen, "по слухам, замешан в антиправительственном заговоре, но доказательств нет.", "据传卷入反政府阴谋，但没有证据。", date));
	...
	private void InvestInProductionEvent(Persona citizen, int slot)
	{
		...
		if ((num == 3 || num == 4 || num == 5) && citizen.status == Job.CorporationOwner && citizen.Wealth >= 10 && num2 < 70 && UnityEngine.Random.value < 0.1f)
		{
			...
			int[] date = CurrentDate();
			citizen.changeLog.Add(FormatLog(citizen, "инвестировал в местное производство, улучшив жизнь населения.", "投入地方生产，改善民众生活水平。", date));
	...
	private void ForbiddenLiteratureEvent(Persona citizen, int slot)
	{
		...
		if (UnityEngine.Random.value < 0.7f)
		{
			...
			citizen.changeLog.Add(FormatLog(citizen, "тайно читал западную литературу, проникся новыми идеями.", "暗中阅读西方书刊，受到新思想启发。", date));
		}
		else
		{
			citizen.status = Job.Prisoned;
			citizen.changeLog.Add(FormatLog(citizen, "пойман за чтением запрещённой литературы!", "被抓到在阅读禁书！", date));
		}
	}
	...
	private void TryCorruptionEvent(Persona citizen, int slot)
	{
		...
		if (num2 && num >= 60 && citizen.Intrigue >= 6 && flag && !(UnityEngine.Random.value > 0.1f))
		{
			...
			if (UnityEngine.Random.value < 0.7f)
			{
				...
				citizen.changeLog.Add(FormatLog(citizen, "тайно присвоил государственные средства.", "暗中侵吞国家资金。", date));
			}
			else
			{
	...
	private void ProtestEvent(Persona citizen, int slot)
	{
		...
		if (num3 && flag && flag2 && UnityEngine.Random.value < 0.1f)
		{
			...
			{
				citizen.status = Job.Prisoned;
				citizen.changeLog.Add(FormatLog(citizen, "был арестован за участие в протестах!", "因参加抗议活动被捕！", date));
			}
			else if (value < 0.8f)
			{
				citizen.status = Job.Underground;
				citizen.Intrigue = Mathf.Min(citizen.Intrigue + 1, 10);
				citizen.changeLog.Add(FormatLog(citizen, "ушёл в подполье после участия в протестах.", "参加抗议后潜伏起来。", date));
			}
			else
			{
				citizen.changeLog.Add(FormatLog(citizen, "участвовал в протестах, но избежал последствий.", "参与抗议却躲过了后果。", date));
			}
		}
	}
	...
	private void CheckFalseAccusationEvent(Persona citizen, int slot)
	{
		...
		if ((num == 0 || num2 == 16) && citizen.Intrigue < 3 && citizen.Charisma < 4 && !citizen.isPolitic && UnityEngine.Random.value < 0.05f)
		{
			...
			{
				citizen.Charisma = Mathf.Min(citizen.Charisma + 1, 10);
				citizen.changeLog.Add(FormatLog(citizen, "оправдался перед следствием, доказал невиновность.", "洗清罪名，证明无罪。", date));
			}
			else
			{
				citizen.status = Job.Prisoned;
				citizen.Wealth = Mathf.Max(citizen.Wealth - 1, 0);
				citizen.changeLog.Add(FormatLog(citizen, "попал под арест по ложному обвинению.", "因诬陷而被监禁。", date));
			}
		}
	}
	...
	private void DoubtIdeologyEvent(Persona citizen, int slot)
	{
		if ((citizen.primaryTrait == PrimaryTrait.LeftRadical || citizen.primaryTrait == PrimaryTrait.Reformist) && globalScript.gameState.data[5] < 300 && UnityEngine.Random.value < 0.2f)
		{
			citizen.primaryTrait = PrimaryTrait.Moderate;
			int[] date = CurrentDate();
			citizen.changeLog.Add(FormatLog(citizen, "начал сомневаться в своих убеждениях.", "开始怀疑自己的信仰。", date));
	...
	private void PraiseAtOfficialEvents(Persona citizen, int slot)
	{
		if (citizen.Charisma >= 8 && globalScript.gameState.data[3] >= 700 && !citizen.tertiaryTraits.Contains(TertiaryTrait.Idol) && citizen.status == Job.RegionalPartyBranchChief && !(UnityEngine.Random.value > 0.1f))
		{
			...
			int[] date = CurrentDate();
			citizen.changeLog.Add(FormatLog(citizen, "стал любимцем местных властей и народа.", "成为当地当局与民众的宠儿。", date));
	...
	public bool TryAmnesty(int slot)
	{
		...
		persona.changeLog.Add(FormatLog(persona, "был амнистирован!", "获得特赦！", CurrentDate()));
		return true;
	}
	...
	public bool TryPursue(int slot)
	{
		...
		if (persona.isPolitic)
		{
			persona.changeLog.Add(FormatLog(persona, "уклонился от преследования, используя своё влияние", "借助政治关系躲过了起诉", CurrentDate()));
		}
		else if (persona.status == Job.Amnestied || persona.status == Job.SignificantPartyMember || persona.status == Job.RegionalPartyBranchChief || persona.status == Job.LocalPartyBranchChief)
		{
			...
			persona.changeLog.Add(FormatLog(persona, "стал объектом политического преследования и смог уйти в подполье", "成为政治迫害的目标，并设法潜伏起来。", CurrentDate()));
		}
		else
		{
			persona.status = Job.Prisoned;
			persona.changeLog.Add(FormatLog(persona, "стал объектом политического преследования и был заключен в тюрьму", "成为政治迫害的目标，并被关押。", CurrentDate()));
		}
		return true;
	}
	...
	public bool TryFinanceSupport(int slot)
	{
		...
		persona.changeLog.Add(FormatLog(persona, "получил финансовую поддержку от государства", "获得国家的财政支持", CurrentDate()));
		return true;
	}
