...
public class Party_zapret : MonoBehaviour
{
	...
	private void OnMouseEnter()
	{
		...
		else
		{
			...
			GetComponent<OkoshkoScript>().text_en = "使派系实力降低15%|<color=yellow>需要超过50%的玩家投票（不是派系数量）\n</color>|<color=" + (global1.gameState.coopAttacked ? "red>等待下一年" : "green>每年仅可使用一次") + "</color>";
		}
	}
			...
			if (GlobalScript.inst.gameState.is_party_enabled[this_number] && GlobalScript.inst.gameState.data[1] > 0 && GlobalScript.inst.gameState.data[53] < 4 && (GlobalScript.inst.gameState.data[15] <= 7 || GlobalScript.inst.gameState.data[15] <= 7 || (GlobalScript.inst.gameState.data[15] > 7 && this_number != 1)))
			{
				...
				if (GlobalScript.inst.gameState.data[15] > 7)
				{
					...
					if (GlobalScript.inst.gameState.data[53] >= 4)
					{
						...
						for (int j = 1; j <= 5; j++)
						{
							GameObject.Find("派系（）" + j + ")").transform.Find("Text").GetComponent<Politic_party_name_show>().Repaint();
							GameObject.Find("派系（）" + j + ")").transform.Find("Znack").GetComponent<Party_ally_script>().Repaint();
							GameObject.Find("派系（）" + j + ")").transform.Find("Znakc（1）").GetComponent<Party_zapret>().Repaint();
						}
					}
				}
				else
