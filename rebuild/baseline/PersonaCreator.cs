...
public class PersonaCreator : MonoBehaviour
{
	...
	private void InitializePrimaryTraitDropdown()
	{
		...
		List<string> list = new List<string>();
		list.Add(translateScript.GetTranslation("未选择", "Не выбран"));
	...
	private void ShowLogPanel(int slot)
	{
		...
		Debug.Log(string.Format("渲染完槽位 {0} 后：face_type={1}，face_parts=[{2}]，jacket={3}", slot, persona.face_type, string.Join(",", persona.face_parts), persona.jacket));
	...
	public void UpdateStatUI()
	{
		...
		for (int i = 0; i < 3; i++)
		{
			if (globalScript.gameState.citizens[i] != null)
			{
				...
				Debug.Log(string.Format("槽位 {0}：face_type={1}，face_parts=[{2}]，jacket={3}", i, persona.face_type, string.Join(",", persona.face_parts), persona.jacket));
				...
				if (!isPortraitRendered[i])
				{
					...
					Debug.Log(string.Format("渲染完槽位 {0} 后：face_type={1}，face_parts=[{2}]，jacket={3}", i, persona.face_type, string.Join(",", persona.face_parts), persona.jacket));
